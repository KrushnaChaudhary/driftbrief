...
const excludedDirs = new Set(['.git', '.driftbrief', 'node_modules', 'vendor', 'dist', 'build', 'coverage', '.next', '.nuxt', '.venv', 'venv', '__pycache__', '.cache', '.tox', 'target', '.idea', '.codex', '.claude', '.cursor', '.agents']);
...
export async function inventory(root: string, signal?: AbortSignal): Promise<{ files: string[]; omissions: Record<string, number> }> {
  ...
  if (gitFiles !== null) {
    ...
    const walk = async (dir: string, parentRules: Array<{ base: string; rules: ReturnType<typeof ignore> }>, depth = 0) => {
      ...
      for (const entry of entries.sort((a,b) => a.name.localeCompare(b.name))) {
        ...
        if (entry.isSymbolicLink() || !eligible(rel) || extra.ignores(rel) ||
  ...
  for (const file of [...new Set(candidates)].sort()) {
    signal?.throwIfAborted();
    if (!eligible(file) || extra.ignores(file)) { skip('excluded'); continue; }

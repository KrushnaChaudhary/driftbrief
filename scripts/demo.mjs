...
</style><main><header><b>◈ DriftBrief</b><span>LOCAL · ONE MAP TOOL</span></header><h1>Help AI understand your game<br>before it starts building.</h1><p>A compact, current map of scripts, scenes and assets—so architecture plans and new systems can start from the project already there.</p><nav id="engines"></nav><section><div class="note">AGENT REQUEST</div><code id="query"></code><p id="metrics"></p></section><section><h2>Connections worth following</h2><div id="links"></div></section><section><h2>Read these files next</h2><div class="nodes" id="nodes"></div></section><p class="note">These are actual outputs from synthetic, reproducible source-format fixtures. Binary asset paths are visible; Unreal Blueprint graphs and live editor state are not parsed. Candidate links are navigation hints, not proven calls.</p><footer>Default map budget: 4,000 UTF-8 bytes, including metadata. Token counts are estimates; no task-level savings claim. Run npm run demo to reproduce. Planning to add Godot next.</footer></main><script>
const examples=${data};
